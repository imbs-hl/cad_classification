...
makeClusterFunctionsSlurm = function(template = "slurm",
  ...
  submitJob = function(reg, jc) {
    ...
    res = runOSCommand("sbatch", shQuote(outfile), nodename = node)
  ...
  listJobsQueued = function(reg) {
    ...
    if (array.jobs)
      cmd = c(cmd, "-r")
    batch.ids = runOSCommand(cmd[1L], cmd[-1L], nodename = node)$output
    if (!is.null(clusters))
...
cluster.functions = makeClusterFunctionsSlurm(template = slurmtmpl_file)
